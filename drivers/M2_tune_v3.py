...
def M2_tune_v3(laser,target_wavelength,timestamp_str):
    ...
    wait_time_while_tuning = 15 # time [sec] for laser to attempt tuning before polling status 

    #wait_time_while_maintaining = 2 # [sec] to allow settling
    
    maintaining_tolerance=0.01
    
    wl_str=str(target_wavelength)
    
    fpath_csv = './laser_log/'+timestamp_str+'wl_'+wl_str+'_log_.csv'
    ...
    np.savetxt(fpath_csv, laser_log, delimiter=",",fmt="%s",newline = "\n")
