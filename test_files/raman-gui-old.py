...
def exp_spad(spad_obj,intTime):
    ...
    if not params.test_mode:
        spad_obj.integration_time=Q_(params.spad_intTime,'ms')
    ...
    for t in timevec:
        if not params.test_mode:
            ...
            if params.power_monitoring:
                ...
                while time.time()<time_start+1: #collect ~1s worth of power readings, then average
                    meas_power = c_double()
                ...
                if params.test_mode or spad[address] is not None:
                    ...
                    if not params.test_mode:
                        try:
                            ...
                            pass
                    if wl not in measurement[address]:
                        measurement[address][wl]=[]
                        power_measurement[address][wl]=[]
                    buff_power=[]
                    buff_spad=[]
                    ...
                    for n in range(1,nexp+1):
                        ...
                        measurement[address][wl].append(buff_spad)
                        power_measurement[address][wl].append(buff_power)
...
def update_pause(n_clicks,exp):
    if n_clicks % 2 ==0: #is not paused/unpause
        ...
        exposing=no_update
    return children,outline,experiment_status,exposing
...
@app.callback([Input("exposing", "children")],
                ...
                State("current_exposures","children"),
                State('total_exposures','children')])
def collect_data(exposing,experiment_type,experiment_status,current_wl,download_trigger,datalog,integration,current_exposures,total_exposures):
    ...
    if  experiment_status == "Measurement aborted":
        ...
        return tuple(list(return_values.values()))
    elif experiment_generator is None or experiment_status in ['Measurement paused','Measurement finished',"Measurement not in progress\n"]:
        print(experiment_status,'null collect data')
        raise PreventUpdate
        ...
        if float(current_wl)!= float(wavelength): #switched to new wavelength
            ...
            if experiment_type in ['Wavelength Sweep','Rolling Average']:
                return_values['trigger-download'] = str(not(bool(download_trigger)))
        try:
            print(datetime.datetime.now(),'collecting data')
            ...
            if type(e) is StopIteration: #generator finished
                ...
                return_values['current_exposures'] = 0
                return_values['prev_exposures'] = 0
                print('RETURNING',tuple(list(return_values.values())))
                return tuple(list(return_values.values()))
            else: #?
                print(e)
                raise PreventUpdate
...
def update_progress(n,integration,nexp,experiment_status,current_exposures,total_exposures,experiment_type):
        ...
        try:
            ...
            if not params.test_mode:
                if laser.is_on():
                    ...
                    laser_output='OFF'
                    raise Exception('Laser output off during measurement; aborting experiment')
...
def update_wavelength_range(rows,experiment_type,experiment_name,integration,nexp,wl_peak):
    ...
    if experiment_type == "Wavelength Sweep":
        if len(rows)==0:
            ...
            target_wavelengths = list(np.round(np.concatenate([np.arange(rows[i]['start'],rows[i]['stop'],rows[i]['step']) for i in range(len(rows))]),2))
            target_wavelengths=sorted(target_wavelengths, key = lambda x:float(x)) #sort wl_list in ascending order
...
def do_measurement(n_clicks,laser_name,experiment_type,experiment_name,concentration,measure_text,integration,nexp,wlstart,wlstop,wlstep,wlpeak,exposing,datalog):
    global experiment_generator
    global laser
    global spad
    global switch
    ...
    if measure_text=="Begin Measurement":
        ...
        if experiment_type == "Rolling Average":
            nexp=1
        meta['excitation_wavelengths'] = [x for x in target_wavelengths]
        ...
        return_values['data-log'] = str(datalog)
        return_values['exposing']=str(not(bool(exposing)))
        return_values['total_exposures'] =0
...
def download_data(download_trigger,status,experiment_type,metajson,datalog):
    global measurement
    global target_wavelengths
    ...
    print(download,autobackup)
    if target_wavelengths==[] or len(measurement)==0 or not any([download,autobackup]):
        raise PreventUpdate
