...
class Spectrum(object):
    ...
    def plot_wavelengths(self,color=None,label='label',mode='lines',wavelength=False,solo=False,show=False):
        if wavelength:
            xaxis='Wavelength (nm)'
            xlabel = 'Wavelength'
        else:
    ...
    def plot_all_traces(self,wavelength=False,show=False):
        ...
        if show:
            fig.show()
        return fig
    def plot_samples(self,show=False):
        pass

def sweptsource_spec_from_upload(contents,filename,label):
    meta={}
    ...
    line =fspec.readline()
    while line.split(',')[0]!= "spad_integration_time":
        fpower.readline()
        meta[line.split(',')[0]] = line.split(',')[1]
        line=fspec.readline()
    fpower.readline()
    meta[line.split(',')[0]] = line.split(',')[1]
    spec = pd.read_csv(fspec)
    #print(spec)
    power=pd.read_csv(fpower)
    #print(power)
    data = {'Wavelength':[x for x in spec.columns[1:]],
                'Raman_Shift':[(1/float(x)-1/float(meta['filter_wavelength']))*1e7 for x in spec.columns[1:]], #in cm^-1
                'Spectrum':[],
                'Avg_Power':[]}
    for i in range(len(spec)):
        spec_buffer = []
        power_buffer = []
        for w in data['Wavelength']:
            spec_buffer.append(sum([float(x) for x in str(spec[w].iloc[i]).split('~')])) #sum the separate spad integrations
            power_buffer.append([float(x) for x in str(spec[w].iloc[i]).split('~')]) #do NOT sum power measurements
        data['Spectrum'].append(spec_buffer)
        power_buffer=np.array(power_buffer).flatten()
        data['Avg_Power'].append(sum(power_buffer)/len(power_buffer)) #take average
    log ='**Spectrum for ' + filename + ' ('+label+')**      \n      Measurement Date: ' + meta['measurement_time'] + \
                '       \n      Filter Wavelength: ' + str(meta['filter_wavelength']) +' nm'+ \
                '       \n      Acquisition Time: '+str(meta['acquisition_time'])+ ' s'+ \
                '       \n      SPAD Integration: ' + meta['spad_integration_time'] + '       \n'
...
def sweptsource_spec_from_df(specdf,powerdf,experiment_name):
    meta={}
    meta['filename'] =''
    meta['label'] =''
    meta['experiment_name']=experiment_name
    fspec = io.StringIO(specdf.to_csv())
    fpower = io.StringIO(powerdf.to_csv())
    line =fspec.readline()
    while line.split(',')[0]!= "spad_integration_time":
        fpower.readline()
        meta[line.split(',')[0]] = line.split(',')[1]
    ...
    for i in range(len(spec)):
        ...
        power_buffer=np.array(power_buffer).flatten()
        data['Avg_Power'].append(sum(power_buffer)/len(power_buffer)) #take average
    log ='**Spectrum for ' + meta['filename'] + ' ('+meta['label']+')**      \n      Measurement Date: ' + meta['measurement_time'] + \
                '       \n      Filter Wavelength: ' + str(meta['filter_wavelength']) +' nm'+ \
                '       \n      Acquisition Time: '+str(meta['acquisition_time'])+ ' s'+ \
                '       \n      SPAD Integration: ' + meta['spad_integration_time'] + '       \n'
    ref=np.mean(np.array(data['Spectrum']),axis=0) #across cols
...
def jsonify(spectra,multiple_spad=False):
    dictlist = []
    buf=[]
    if multiple_spad:
        for exp in spectra: #spectra is list experiments; each experiment is a list of Spectrum objs generated from each spad measurement
            for spec in exp:
...
def zip_data(measurement_data,power_data,datalog,experiment_name,working_directory):
    ...
    with zipfile.ZipFile(experiment_name+'.zip', "w") as zf:
        for address in measurement_data: #and, trivially, also power data
            ...
            #print(measurement_data[address].head())
            mdf = experiment_name+"_"+str(address.split('::')[3])+".spad"
            pdf = experiment_name+"_"+str(address.split('::')[3])+".power"
            logfile = experiment_name+".log"
            measurement_data[address].to_csv(mdf)
            power_data[address].to_csv(pdf)
        ...
        zf.close()
    if platform == 'darwin':
        os.system("mv "+experiment_name+".zip " + working_directory[:-1]+"/"+experiment_name+".zip"+working_directory[-1])
    else:
        os.system("move "+experiment_name+".zip " + working_directory[:-1]+"/"+experiment_name+".zip"+working_directory[-1])