...
def check_status(datalog):
    ...
    if params.test_mode:
        ...
        spad_status='ERROR'
    if pm is not None:
        pm_status='CONNECTED - TUNED TO {} nm'.format(current_wavelength)
...
def instrument_shutdown(datalog):
    ...
    if switch is not None:
        try:
            ...
            print('Could not close Dicon switch : {}'.format(sys.exc_info()))
        else:
            switch = None
    instrument_status={'laser':laser_status,'pm':pm_status,'spad':spad_status}
    return instrument_status,datalog

def exp_spad(intTime,spad_obj=None):
    ...
    for t in timevec:
        if not params.test_mode:
            ...
            if params.power_monitoring:
                ...
                while time.time()<time_start+params.spad_intTime/1000: #collect ~1 spad integrations worth of power readings, then average
                    meas_power = c_double()
...
def data_stream(measurement,power_measurement, state, prev_state, measurement_parameters ,datalog, failed_wavelengths):
    global laser
    global pm
    global spad

    address= state['current_address']
    if state['exposure_index'] == 1: #measurement is beginning
        if not params.test_mode:
            #resetting spad parameters test
            spad[address].bias = Q_(int(params.spad_bias*1e6), 'uV')
            spad[address].threshold = Q_(int(params.spad_threshold*1e6), 'uV')
            #spad[address].set_temp = Q_(int(params.spad_temp),'millidegC')
            spad[address].run = True
            ...
            if not laser.is_on():
               ...
               # raise Exception("Laser output off during experiment")
               pass
    if address != prev_state['current_address']: # new address
        ...
        if not params.test_mode:
            try:
                ...
                datalog.add('change switch channel failed')
            else:
                datalog.add('set channel to '+str(params.spad_addresses[address]))
        else:
            datalog.add('set channel to '+str(params.spad_addresses[address]))
    wavelength=state['current_wl']
    if wavelength != prev_state['current_wl']: #new wavelength
        datalog.add('tuning laser to {}'.format(state['current_wl']))
        if not params.test_mode:
            tune_success=laser.set_wavelength(wavelength)
            datalog.add('tune success:'+str(tune_success))
            if params.power_monitoring:
                power=c_double()
                pm.setWavelength(c_double(wavelength))
        ...
        if not tune_success:
            ...
            print('TUNING FAILED: skipping '+str(wavelength)+' nm')
            failed_wavelengths.append(wavelength)
        else:
            datalog.add('set laser to '+str(wavelength)+' nm')
            print('set laser to '+str(wavelength)+' nm')
            print(measurement)
            measurement[address][str(wavelength)]=[]
            power_measurement[address][str(wavelength)]=[]
        #Delay to make sure readings consistent after wavelength adjustment
        sleep(params.measurement_delay)
    #Do Exposure
    if str(wavelength) in measurement[address]:  #if wl not in dict then tuning failed; skip
        buff_spad,buff_power,time_exposed=exp_spad(measurement_parameters['integration_time'],spad[address])
        measurement[address][str(wavelength)].append(buff_spad)
        power_measurement[address][str(wavelength)].append(buff_power)
        print('exposed spad!',buff_spad)
        #print('MEASUREMENT',measurement)
        #print('ADDRESS',address)
        #print('WAVELENGTH',wavelength, str(wavelength) in measurement[address].keys())
        datalog.add('Acquisition {} of {}'.format(state['exposure_index'],measurement_parameters['total_exposures']) + ' - spad exposed for {} s'.format(time_exposed))
        print('Acquisition {} of {}'.format(state['exposure_index'],measurement_parameters['total_exposures']) + ' - spad exposed for {} s'.format(time_exposed))
    prev_state['exposure_index'] = state['exposure_index']
    state['exposure_index'] += 1
    prev_state['current_exposure'] = state['current_exposure']
    state['current_exposure'] +=1
    prev_state['current_address'] = state['current_address']
    prev_state['current_wl'] = state['current_wl']
    if state['exposure_index'] > measurement_parameters['total_exposures']: #all loops done
        print('all loops done!')
        raise StopIteration
    if state['current_exposure']>measurement_parameters['nexp']: #finished with one wavelength of measurements
        state['current_exposure'] = 1
        wl_index = measurement_parameters['target_wavelengths'].index(state['current_wl'])
        try:#first try incrementing wavelength (inner loop)
            state['current_wl'] = measurement_parameters['target_wavelengths'][wl_index + 1]
        except: #next try incrementing address (outer loop)
            state['current_wl'] = measurement_parameters['target_wavelengths'][0]
            address_index = measurement_parameters['spad_addresses'].index(state['current_address'])
            state['current_address'] = measurement_parameters['spad_addresses'][address_index+1]
    #print('STATE',state)
    #print('PREV_STATE',prev_state)
    return measurement,power_measurement,state,prev_state,datalog,failed_wavelengths

def process_data(measurement,power_measurement,metadata,filter_complete=True):
    ...
    for address in measurement:
        ...
        #print('METADAT',type(meta_df[address]['excitation_wavelengths']),[x for x in meta_df[address]['excitation_wavelengths']])
...
@app.callback([Output('file-list', 'data'),
              ...
              Input('load-peak-demo','n_clicks'),
              Input('load-sweep-demo','n_clicks')],
              [State('spectra','children'),
              State('original-spectra','children'),
              State('upload-data', 'filename'),
              State('upload-data', 'last_modified')])
def show_files(list_of_contents, metadata_cols,peak_clicks,sweep_clicks,json_spectra, json_original_spectra, list_of_names, list_of_dates):
    ...
    if json_spectra is not None:
        ...
        print('SHOWMETA',spectra[0].meta['integration'],len(spectra[0].ref))
    else:
        spectra=[]
    ...
    elif peak_clicks and trigger=='load-peak-demo':
        spectra = batch_process_folder('demo_data/demo-peak')
        original_spectra = spectra.copy()
    elif sweep_clicks and trigger=='load-sweep-demo':
        spectra = batch_process_folder('demo_data/demo-sweep')
        original_spectra = spectra.copy()
...
def update_current(json_spectra,graphnum,clickData,ydata_switchval,processing_switchval,spectral_units):
    ...
    if processing_switchval:
        mode='lines+markers'
        print('EXPERIMENT TYPE',spectra[g].meta['experiment_type'])
...
@app.callback(Input("progress-interval","n_intervals"),
              [Output("exposing","children"),
              Output("prev_state","children"),
              Output("state_cache","children"),
              Output("data-log","children")],
              [State("exposing","children"),
              State("state","children"),
              State("measurement_parameters","children"),
              State("prev_state","children"),
              State("status","children"),
              State("state_cache","children"),
              State("data-log","children")])
def control_exposure(n,exposing,state,measurement_parameters,prev_state,status,cache,datalog):
    state = json.loads(state)
    prev_state = json.loads(prev_state)
    cache = json.loads(cache)
    if cache == state or state == {}: #don't execute if no change yet
        raise PreventUpdate
    measurement_parameters = json.loads(measurement_parameters)
    if params.verbose_log:
        datalog=Datalog(datalog)
        datalog.add('called control_exposure()')
        datalog = str(datalog)
    else:
        datalog = no_update
    if status == "Measurement aborted" or state['exposure_index']<=measurement_parameters['total_exposures'] and state['current_exposure']>0 and prev_state['exposure_index'] != state['exposure_index']:
        print('triggering data collection')
        prev_state['current_exposure'] =state['current_exposure']
        prev_state['exposure_index'] =state['exposure_index']
        return str(not(bool(exposing))),json.dumps(prev_state), json.dumps(state),datalog #switch to trigger data collection
    else:
        raise PreventUpdate
...
@app.callback([Input("exposing", "children")],
                ...
                Output('prev_state','children'),
                Output('measurement','children'),
                Output('power_measurement','children'),
                ...
                State("state","children"),
                State("prev_state","children"),
                State("measurement","children"),
                State("power_measurement","children"),
                State("failed_wavelengths","children")])
def collect_data(exposing,experiment_type,experiment_status,download_trigger,measurement_parameters,datalog,state,prev_state,measurement,power_measurement,failed_wavelengths):
    datalog=Datalog(datalog)
    if params.verbose_log:
        datalog.add('calling collect_data()')
    measurement= json.loads(measurement)
    power_measurement = json.loads(power_measurement)
    state = json.loads(state)
    prev_state = json.loads(prev_state)
    measurement_parameters = json.loads(measurement_parameters)
    failed_wavelengths = json.loads(failed_wavelengths)
    TIME = measurement_parameters['integration_time'] * state['exposure_index']
    prev_TIME = measurement_parameters['integration_time'] * (prev_state['exposure_index'])
    return_values = {'status':no_update,
                     'trigger-download':no_update,
                     'data-log':no_update,
                     'state':no_update,
                     'prev_state':no_update,
                     'measurement':no_update,
                     'power_measurement':no_update,
                     'failed_wavelengths':no_update}
    if experiment_status == "Measurement aborted":
        datalog.add('Abort success')
        return_values['status'] = 'Measurement not in progress'
        return_values['trigger-download'] = str(not(bool(download_trigger)))
        return_values['data-log'] = str(datalog)
        return_values['state'] = '{}'
        return_values['prev_state'] = '{}'
        return tuple(list(return_values.values()))
    elif experiment_status in ['Measurement paused','Measurement finished',"Measurement not in progress\n"]:
        raise PreventUpdate
    elif experiment_status == 'Measurement in progress':
        if prev_state['current_wl']!= state['current_wl']: #switched to new wavelength
            if experiment_type in ['Wavelength Sweep','Rolling Average'] and measurement != init_measurement:
                return_values['trigger-download'] = str(not(bool(download_trigger)))
        try:
            measurement,power_measurement,state,prev_state,datalog,failed_wavelengths = data_stream(measurement,power_measurement,state,prev_state,measurement_parameters,datalog,failed_wavelengths)
            return_values['measurement'] = json.dumps(measurement)
            return_values['power_measurement'] = json.dumps(power_measurement)
            return_values['state'] = json.dumps(state)
            return_values['prev_state'] = json.dumps(prev_state)
            return_values['data-log'] = str(datalog)
            return_values['failed_wavelengths']= json.dumps(failed_wavelengths)
        except Exception as e:
            print(str(e))
            if type(e) is StopIteration: #measurement is finished
                experiment_status="Measurement finished"
                datalog.add(experiment_status)
                return_values['status'] = experiment_status
                return_values['data-log'] = str(datalog)
                return_values['trigger-download'] = str(not(bool(download_trigger)))
                return_values['state'] = '{}'
                return_values['prev_state'] = '{}'
                ...
                return_values['power_measurement'] = json.dumps(power_measurement)
                return tuple(list(return_values.values()))
            elif str(e)=="Laser output off during experiment": #laser output off
                experiment_status="Measurement aborted"
                datalog.add("Laser output off. Aborting measurement.")
                return_values['status'] = experiment_status
                return_values['data-log'] = str(datalog)
                return_values['state'] = '{}'
                return_values['prev_state'] = '{}'
            ...
            #if params.check_wavelength and TIME % params.check_wavelength_interval < prev_TIME % params.check_wavelength_interval:
            if params.check_wavelength and state['current_wl'] not in failed_wavelengths:   
                if params.test_mode:
                    datalog.add('Laser wavelength is {}'.format(state['current_wl']))
                else:
                    measured_wavelength = laser.get_wavelength()
                    datalog.add('Laser wavelength is {}'.format(measured_wavelength))
                    datalog.add('Spad temperature is {}'.format(spad[state['current_address']].temp.magnitude/1e3))
                    #if state['current_wl'] != -1 and abs(state['current_wl']-float(measured_wavelength))>params.lambda_tolerance: #re-tune needed
                        #datalog.add('Wavelength error tolerance exceeded. Retuning laser to {}'.format(state['current_wl']))
                        #if not params.test_mode:
                            #tune_success=laser.set_wavelength(state['current_wl'])
            #Run laser realign check subroutine
            if params.check_alignment and TIME % params.alignment_interval < prev_TIME % params.alignment_interval:
                if params.test_mode:
            ...
            if TIME % params.autobackup_interval < prev_TIME % params.autobackup_interval and state['current_wl'] not in failed_wavelengths:
                if experiment_type in ['Peak Measurement']:
                    return_values['trigger-download'] = str(not(bool(download_trigger)))
            return_values['data-log'] = str(datalog)
            return tuple(list(return_values.values()))
    else: 
        raise Exception('Unknown measurement status')
...
def update_progress(state,measurement_parameters,experiment_status,experiment_type,measurement,power_measurement,datalog):
    ...
    if params.verbose_log:
        datalog=Datalog(datalog)
        datalog.add('called update_progress()')
        return_values['data-log'] = str(datalog)
    if experiment_status=='Measurement in progress':
        try:
            #update progress display
            try:
                progress=int(state['exposure_index']/measurement_parameters['total_exposures'] * 100)
            except:
                progress=0
            address = state['current_address']
            current_channel=int(params.spad_addresses[address])
            ...
            if experiment_type in ['Peak Measurement','Wavelength Sweep']:
                for address in spad:
                    try:
                        flat_measure=np.array(measurement[address][str(wavelength)]).flatten()
                    except:
                        flat_measure=np.array([])
                    try:
                        flat_power=np.array(power_measurement[address][str(wavelength)]).flatten()
                    except:
                        flat_power=np.array([])
            ...
            return_values['progress_label'] = f"{progress} %" if progress >= 5 else ""
            ...
            print(e)
    elif experiment_status == 'Measurement paused':
        try:
            progress=int(state['exposure_index']/measurement_parameters['total_exposures'] * 100)
        except:
            progress=0
        ...
        return_values['progress_label'] = f"{progress} %" if progress >= 5 else ""
        return tuple(list(return_values.values()))
    elif experiment_status == 'Measurement finished':
        progress = 0
        return_values['progress_value'] = progress
        return_values['progress_label'] = f"{progress} %" if progress >= 5 else ""
        return tuple(list(return_values.values()))
...
@app.callback([Input('wl-ranges','data'),
              ...
              Output('measurement_parameters','children')],
              State('measurement_parameters','children'))
def update_wavelength_range(rows,experiment_type,experiment_name,integration,nexp,wl_peak,measurement_parameters):
    measurement_parameters = json.loads(measurement_parameters)
    if experiment_type == "Wavelength Sweep":
        if len(rows)==0:
            target_wavelengths = []
    ...
    measurement_parameters['target_wavelengths'] = [float(x) for x in target_wavelengths]
    if len(target_wavelengths)>0 and experiment_name is not None and integration is not None and nexp is not None:
        ...
        return False,json.dumps(measurement_parameters)
    return True,json.dumps(measurement_parameters)
...
@app.callback(Input('measure','n_clicks'),
            [Output('start-trigger','children'),
             Output('data-log','children')],
            [State('measure','children'),
             State('start-trigger','children'),
             State('data-log','children')]
)
def start_measurement(nclicks,measure_text,start_trigger,datalog):
    #check to make sure instruments are ready for measurement
    print(measure_text)
    if measure_text=="Begin Measurement" and not params.test_mode: #user is trying to begin a measurement
        #reset logfile
        datalog = Datalog()
        instrument_status,datalog=check_status(datalog)
        if params.force_spad_cool and "READY" not in instrument_status['spad']:
            datalog.add('SPAD still needs to cool. Measurement will begin when temperature stabilizes.')
        #turn on laser
        laser.output_on()
    else: #user is trying to stop measurement
        datalog=Datalog(datalog)
    return not(bool(start_trigger)),str(datalog)
...
def do_measurement(start_trigger,measurement_parameters,laser_name,experiment_type,experiment_name,concentration,measure_text,integration,nexp,exp_reps,wlstart,wlstop,wlstep,wlpeak,exposing,datalog):
    global laser
    global spad
    global switch
    global pm
    ...
    return_values = {'status':no_update,
                    'data-log':no_update,
                    'exposing':no_update,
                    'measurement_parameters':no_update,
                    ...
                    'state':no_update,
                    'prev_state':no_update,
                    'measurement':json.dumps(init_measurement),
                    'power_measurement':json.dumps(init_measurement),
                    'laser-status_children':no_update,
                    'spad-status_children':no_update,
                    'power-status_children':no_update}
    if measure_text=="Begin Measurement":
        if not params.test_mode:
            instrument_status,datalog=check_status(datalog)
            ...
            return_values['laser-status_children']=instrument_status['laser']
            return_values['spad-status_children']=instrument_status['spad']
            return_values['power-status_children']=instrument_status['pm']
        #update metadata
        meta['laser'] = laser_name
        if int(exp_reps) == 1:
            meta['experiment_name'] = experiment_name
        else:
            meta['experiment_name'] = experiment_name+'_rep'+str(exp_reps)
        meta['experiment_type']=experiment_type
        meta['integration'] = integration
        meta['concentration']=concentration
        meta['repetitions'] = nexp
        measurement_parameters['nexp'] = nexp
        measurement_parameters['integration_time'] = integration
        measurement_parameters['spad_addresses'] = list(params.spad_addresses.keys())
        measurement_parameters['total_exposures'] = nexp*len(measurement_parameters['target_wavelengths'])*len(params.spad_addresses)
        state = {'current_address':list(params.spad_addresses.keys())[0],'current_wl':measurement_parameters['target_wavelengths'][0],'current_exposure':1,'exposure_index':1}
        prev_state = {'current_address':-1,'current_wl':-1,'current_exposure':1,'exposure_index':1}
        print(state)
        print(prev_state)
        ...
        return_values['status'] = 'Measurement in progress'
        return_values['data-log'] = str(datalog)
        return_values['exposing']=str(not(bool(exposing)))
        print(return_values['exposing'])
        return_values['measure_children'] = "Abort Measurement"
        return_values['measure_color'] = "dark"
        return_values['collected_spectra'] = None
        return_values['metajson'] = meta.to_json()
        return_values['measurement_parameters'] = json.dumps(measurement_parameters)
        return_values['state'] = json.dumps(state)
        return_values['prev_state'] = json.dumps(prev_state)
        return tuple(list(return_values.values()))
    else: #abort measurement
        datalog.add('stopping measurement')
        sleep(3)
        return_values['status'] = "Measurement aborted"
        return_values['data-log'] = str(datalog)
        return_values['exposing']=str(not(bool(exposing)))
        return_values['measurement_parameters'] = '{}'
...
def download_data(download_trigger,status,experiment_type,metajson,datalog,measurement,power_measurement,measurement_parameters,failed_wavelengths,start_trigger,exp_reps):
    measurement = json.loads(measurement)
    power_measurement = json.loads(power_measurement)
    measurement_parameters = json.loads(measurement_parameters)
    failed_wavelengths = json.loads(failed_wavelengths)
    ...
    return_values = {'collected-spectra':no_update,
                   ...
                   'metajson':no_update,
                   'failed_wavelengths':no_update,
                   'start-trigger':no_update,
                   'exp-reps':no_update}
    print('STATUS',status)
    download = status in ["Measurement finished","Measurement aborted"]
    autobackup = params.auto_backup and status =='Measurement in progress'
    print('DOWNLOAD',download,'AUTOBACKUP',autobackup)
    print('download in progress')
    metadata = Metadata(metajson)
    metadata['excitation_wavelengths'] = list(filter(lambda i: i not in failed_wavelengths, measurement_parameters['target_wavelengths']))
    metadata['endtime'] = datetime.datetime.now().isoformat()
    if download or autobackup:
        try:
            if experiment_type == 'Wavelength Sweep':
                measurement_df,power_df,meta_df= process_data(measurement,power_measurement,metadata) #returns dict of dicts indexed by SPAD address
            else:
                measurement_df,power_df,meta_df = process_data(measurement,power_measurement,metadata,filter_complete=False)
    ...
    if download: #measurement is finished;serve notification
        ...
        print('exp-reps:',exp_reps)
        return_values['measure_children'] = "Begin Measurement"
        return_values['measure_color'] = "primary"
        if int(exp_reps) == 1:
            return_values['modal_body'] = meta_df[next(iter(meta_df))]['experiment_name']+ " data zipped to " + params.working_directory
            ...
            #trigger start of next experiment repetition
            return_values['start-trigger'] = not(bool(start_trigger))
            return_values['exp-reps'] = int(exp_reps)-1
        return_values['data-log'] = str(datalog)
