...
from gui_components.gui_layout_old import *
...
def add_column(n,cols,metadata_cols,new_col,json_spectra,json_original_spectra):
    cols.append({
...
@app.callback(Input('mod-data','n_clicks'),
              [Output('spectra','children'),
                Output('log','children'),
                Output('file-list', 'data'),
                Output('file-list','selected_rows'),
              ...
              [State('mod-selection','value'),
               State('spectra','children'),
               State('original-spectra','children'),
              State('datatable-columns','value')])
def modify_data(n, mod_selection,json_spectra,json_original_spectra,metadata_cols):
    return_values = {'spectra':no_update,
                     ...
                     'file-list_style':no_update,
                     'rebin':no_update,
                     'remove_samples':no_update,
                     'remove_wavelengths':no_update}
    if mod_selection == 'revert to original':
        ...
        return_values['log'] = str(Processlog([s.log for s in original_spectra]))
        return_values['file-list_data']=data
        return_values['file-list_selected']=[x for x in range(len(data))]
        return_values['file-list_style']=style_data_conditional
    elif mod_selection =='reshape data':
        return_values['rebin'] = True
    elif mod_selection == 'remove samples':
        return_values['remove_samples'] = True
    elif mod_selection == 'remove wavelengths':
        return_values['remove_wavelengths'] = True
    return tuple(list(return_values.values()))
...
@app.callback([Output('remove_samp_dialogue','children'),
               ...
               [State('file-list','selected_rows'),
                ...
                State('datatable-columns','value')])
def toggle_remove_samples(n,selected_rows,rows,json_spectra,samples_to_remove,metadata_cols):
...
@app.callback([Output('remove_wl_dialogue','children'),
               ...
               [State('file-list','selected_rows'),
                ...
                State('datatable-columns','value')])
def toggle_remove_wavelengths(n,selected_rows,rows,json_spectra,wls_to_remove,metadata_cols):
...
@app.callback([Output('working','figure')],
              [Input('custom-plot','n_clicks')],
              [State('working','clickData'),
               State('xaxis','value'),
               State('spectra','children')])
def custom_plot(n,clickData,xaxis,json_spectra):
    if json_spectra is not None:
        spectra = spec_from_json(json_spectra)
    else:
        raise PreventUpdate
    raman = clickData['points'][0]['x']
    index_point = spectra[0].meta['excitation_ramanshifts'].index(raman)
    x = []
    for s in spectra:
        try:
            x.append(float(s.meta[xaxis]))
        except Exception as e:
            print(e)
            x.append(0)
    refs = np.array([x.ref[index_point] for x in spectra])
    noise = np.array([x.noise[index_point] for x in spectra])
    fig=go.Figure(data=go.Scatter(x=x,y=refs,
                                  error_y=dict(
                                            type='data',
                                            array=noise,
                                            visible=True
                                  ),mode='markers'))
    fig.update_layout(title = f"SPAD Count vs {xaxis} at {raman} cm^-1",
                        xaxis_title = xaxis,
